...
function _Sweep()
    ...
    local StoreReadingBufferIndexes = function()
        ...
        local SetupLastPoints = function(testLevel, i, selSmuIndex)
            ...
            elseif (testLevel == sweepIndex) then
                if (highSpeedSampling == true) then
                    incVal = (1 * #NumPoints * 60)
                else
                    incVal = (1 * #NumPoints * 1)
                end
            end
    ...
    delay(.1)
    local reading = smua.measure.i() * #NumPoints
	...
	if (reading<10e-9) then
          ...
          smua.source.output = 1
          delay(.1)
          reading = smua.measure.i() * #NumPoints
          if (reading > rangei) then
       		rangei = 1e-6
        	smua.measure.rangei = rangei
        	smua.source.levelv = .001
...
if (additional=="Joule") then 
    if (doCap==1) then 
        ...
        smua.source.rangei = 10e-9
        smua.source.leveli = 1e-9
        ...
        for v=0,100 do
            delay(.5)
            readingV =( smua.measure.v() + smua.measure.v() + smua.measure.v() + smua.measure.v() )/4
            readingI =( smua.measure.i() + smua.measure.i() + smua.measure.i() + smua.measure.i() )/4
            time = os.date("*t")
            print(0, 1, 0,60*time.min+ time.sec + v*.5 , 0, 0, 0,readingV,readingI)
        end
        smua.source.output = smua.OUTPUT_OFF
    else 
        for c=1,2 do
